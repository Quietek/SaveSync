...
if (__name__ == "__main__"):
    ...
    #We check if the SaveSync database has been initialized, and if it hasn't flag that this is a first run
    if not os.path.isfile('./saves.db'):
        FirstRunFlag = True
        InitializeDatabase()
    #We check if a custom configuration path was specified, and if not default to the file created at $HOME/.config/SaveSync/config
    if ConfigPath == '':
        ConfigPath = os.path.expanduser('~') + '/.config/SaveSync/config'
    ...
    #Help menu, displays information on how to use SaveSync
        ...
        print('GameID\'s are a SaveSync specific value that are generated each time a new non-Steam game is added.')
        ...
        print('By default, SaveSync will run in interactive mode when no command line arguments are passed or if the command line arguments passed couldn\'t be parsed.')
        ...
        print('         WARNING: The save rollback feature is still actively being tested and developed, while it shouldn\'t delete your saves backed up with SaveSync, it will overwrite Game Saves on your PC. Use at your own risk.')
        ...
        print('==============================================================================================================')
        print('UPDATES:')
        print('There currently isn\'t an automated function to check or update SaveSync.')
        print('If the version you are using appears to be broken, check https://www.github.com/Quietek/SaveSync to manually update SaveSync.')
        print('To manually update, simply download the newest release from github, extract, and copy and overwrite into your current SaveSync directory.')
